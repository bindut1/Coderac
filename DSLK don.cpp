...
void insert(node **head, int k, int x){
    ...
    if(k==1) push_front(head, x);
        ...
        node *new_node=add_node(x);
        new_node->next=tmp->next; //phan next cua new_node luu dia chi cua node thu k;
        tmp->next=new_node; //phan next cua node thu k-1(tmp) luu dia chi cua node moi them vao
    }
}
